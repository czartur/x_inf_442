...
RandomProjection::RandomProjection(int original_dimension, int col_class, int projection_dim, std::string type_sample) {
    ...
    int d = original_dimension;
    double f = std::sqrt(3.f/original_dimension);
    m_projection = type_sample == "Gaussian" ? RandomGaussianMatrix(d, projection_dim) : f*RandomRademacherMatrix(d, projection_dim).cast<double>();
}
