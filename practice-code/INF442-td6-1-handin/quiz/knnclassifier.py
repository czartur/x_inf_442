...
def train_and_evaluate(train_data, test_data, class_col, nneighbors, roc_fname="roc.png"):
    ...
    # X_train, X_test, y_train, y_test  = np.asarray(train_data.iloc[:, 1:]), np.asarray(test_data.iloc[:, 1:]), np.asarray(train_data.iloc[:,0]), np.asarray(test_data.iloc[:,0]) 
    # cls = KNeighborsClassifier(n_neighbors=nneighbors, algorithm='kd_tree')
...
if __name__ == "__main__":
    ...
    norm_train_data, norm_test_data = normalize(train_data, test_data, 'Risk')
    # train_and_evaluate(train_data, test_data, 'Risk', N)
    # train_and_evaluate(norm_train_data, norm_test_data, 'Risk', N)

    for N in N_list:
        train_and_evaluate(norm_train_data, norm_test_data, 'Risk', N, roc_fname="roc" + str(N) + ".png")
